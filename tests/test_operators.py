...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert sigmoid(a + 1e-3) >= s


@pytest.mark.task0_2
@given(lists(small_floats, min_size=3, max_size=3, unique=True))
def test_transitive(vals: list[float]) -> None:
    a, b, c = sorted(vals)
    assert lt(a, b) == 1.0
    assert lt(b, c) == 1.0
    assert lt(a, c) == 1.0
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    ls1_sum = sum(ls1)
    ls2_sum = sum(ls2)
    assert_close(add(ls1_sum, ls2_sum), sum(addLists(ls1, ls2)))
